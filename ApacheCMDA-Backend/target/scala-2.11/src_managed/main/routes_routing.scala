...
// @HASH:d01df51938ac55d75674004dc49fe83bf5c136f8
// @DATE:Wed Dec 02 00:18:18 PST 2015
...
// @LINE:95
private[this] lazy val controllers_SearchController_searchUserFuzzy61_route = Route("GET", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("search/user/fuzzy/"),DynamicPart("partial", """[^/]+""",true))))
private[this] lazy val controllers_SearchController_searchUserFuzzy61_invoker = createInvoker(
...
// @LINE:96
private[this] lazy val controllers_SearchController_searchPostContentByKeyword62_route = Route("GET", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("search/post/"),DynamicPart("keyword", """[^/]+""",true))))
private[this] lazy val controllers_SearchController_searchPostContentByKeyword62_invoker = createInvoker(
...
// @LINE:95
case controllers_SearchController_searchUserFuzzy61_route(params) => {
   call(params.fromPath[String]("partial", None), Param[String]("format", Right("json"))) { (partial, format) =>
        controllers_SearchController_searchUserFuzzy61_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.SearchController]).searchUserFuzzy(partial, format))
   }
}
        

// @LINE:96
case controllers_SearchController_searchPostContentByKeyword62_route(params) => {
   call(params.fromPath[String]("keyword", None), Param[String]("format", Right("json"))) { (keyword, format) =>
        controllers_SearchController_searchPostContentByKeyword62_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.SearchController]).searchPostContentByKeyword(keyword, format))
   }
}
